...
const Hotspot = {
  ...
  gateLock(h) {
    ...
    Modal.open(`<h2>铜锁</h2>
      ...
      <p class="puzzle-hint">提示：沈家灭亡的那一年……老宅石碑上似乎有线索。</p>`);
    $('p-ok').onclick = () => {
  ...
  livingPainting(h) {
    ...
    Modal.open(`<h2>油画</h2>
      ...
      <p class="puzzle-hint">密码是三位数……你需要找到线索。</p>
      <div class="puzzle-row">
  ...
  livingFireplace(h) {
    ...
    msgFade('你在壁炉的灰烬中翻找，发现了一张未烧尽的纸片。上面写着：\n「挂钟停在……三时十五分……那是她最后一次……」\n\n三时十五分。3:15。', 'warn');
  ...
  diningClock(h) {
    ...
    Modal.open(`<h2>老式挂钟</h2>
      ...
      <p>但你能拨动指针。拨到什么时刻？</p>
      <p class="puzzle-hint">输入时间，格式 H:MM 或 HH:MM</p>
      <div class="puzzle-row">
  ...
  shrineBurner(h) {
    ...
    Modal.open(`<h2>香炉</h2>
      ...
      <p class="puzzle-hint">提示：灵位上的排列顺序就是长幼顺序。</p>
      <div class="puzzle-row">
        <input id="p-input" maxlength="5" placeholder="?????">
        <button id="p-ok">确认</button>
      </div>`);
  ...
  wellWall(h) {
    if (flag('wellWall_read')) { msg('井壁上的刻痕你已经记下了。它们与地下室的符号似乎有关联。'); return; }
    flag('wellWall_read', true);
    msgFade('你仔细查看井壁上的刻痕——青苔下隐约可见古老的符号。\n这些符号与地下室的完全一致，但排列方式不同，似乎在指示某种机关的位置。\n\n也许地下室墙壁上也有类似的线索……', 'warn');
    h_found(h);
  },
  ...
  if (type === 'true') {
    ...
    setTimeout(() => {
      Modal.open(`<h2 class="ending-good">· 真 相 ·</h2>
        ...
        <div style="text-align:center;margin-top:16px;"><button class="act" id="restart">重新开始</button></div>`);
...
Game.showIntro = function() {
  ...
  let continueBtn = '';
  if (hasSave) {
    continueBtn = '<button class="act" id="intro-continue" style="margin-right:12px;">继 续 游 戏</button>';
  }
  Modal.open(`<h2>雾 中 宅</h2>
    <p style="text-align:center;color:var(--gold);letter-spacing:3px;font-size:13px;">THE HOUSE IN THE MIST</p>
    <p style="margin-top:16px;">你的好友林默是一名民俗学者。他来信说在深山中发现了一座荒废的老宅——雾隐宅，沈氏家族曾在其中居住，却在百年前一夜之间全部失踪。</p>
    <p>林默前去调查后音讯全无。你循着他的足迹，来到了这座被浓雾包裹的老宅门前……</p>
    <p style="color:var(--gold);text-align:center;font-style:italic;margin-top:14px;">「找到日记碎片，拼凑出真相。小心那面镜子。」</p>
    <p class="puzzle-hint" style="text-align:center;">点击场景中发光的区域进行调查。收集物品和日记残页，揭开雾隐宅的秘密。</p>
    <div style="text-align:center;margin-top:20px;">
      ${continueBtn}
      <button class="act" id="intro-start">推 开 铁 门</button>
    </div>`);
  if (hasSave) {
